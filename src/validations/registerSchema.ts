...
const mappedEspecialidades = especialidades.map((esp) => esp.nombre) as [string, ...string[]]
...
export const registerSchema = z
    .object({
        ...
        especialidad: z.enum(mappedEspecialidades, {
            errorMap: () => ({ message: 'Por favor selecciona una especialidad' }),
        }),
        location: z.string().min(4, { message: 'Mínimo 4 caracteres' }),
